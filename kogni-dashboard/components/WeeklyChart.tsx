...
const CustomTooltip = ({ active, payload }: any) => {
  ...
  return (
    <div style={{
      ...
      fontSize: 7, color: "#fff", lineHeight: 2,
    }}>
      <div style={{ color: "#888" }}>{d.fullDate}</div>
...
function toLocalDateKey(dateStr: string): string {
  // Convert any date string to YYYY-MM-DD in LOCAL time
...
export function WeeklyChart({ scores }: Props) {
  ...
  // Deduplicate: keep HIGHEST fatigue score per calendar day
  // (shows worst reading of the day, most informative)
  const byDay = new Map<string, { score: number | null; dateStr: string }>();

  scores.forEach(s => {
    const dayKey  = toLocalDateKey(s.date);
    const existing = byDay.get(dayKey);
    const thisScore = s.fatigue_score;

    if (!existing) {
      byDay.set(dayKey, { score: thisScore, dateStr: s.date });
    } else {
      // Keep the latest score (most recent run of score_now)
      if (new Date(s.date) > new Date(existing.dateStr)) {
        byDay.set(dayKey, { score: thisScore, dateStr: s.date });
      }
    }
  });

  // Sort by date, take last 7 unique days
  const sortedDays = Array.from(byDay.entries())
    .sort(([a], [b]) => a.localeCompare(b))
    .slice(-7);

  const data = sortedDays.map(([dayKey, { score }]) => {
    ...
    return { day: dayName, fullDate, score, isToday };
  });

  // Pad to 7 bars
  while (data.length < 7) {
    data.unshift({ day: "---", fullDate: "No data", score: null, isToday: false });
  }

  return (
    <div style={{
      background: "#0d1117", border: "2px solid #000",
      padding: "12px 8px 8px", position: "relative", overflow: "hidden",
    }}>
      <div style={{
